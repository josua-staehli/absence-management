...
internal sealed class AbsenceRequestConfiguration : IEntityTypeConfiguration<AbsenceRequest>
{
    ...
    public void Configure(EntityTypeBuilder<AbsenceRequest> builder)
    {
        ...
        // No foreign key: the employee lives in another module's database, so the constraint
        // cannot exist even in principle. That the id belongs to an employee is established once,
        // when the request is created, by asking IEmployeeDirectory. The index stays, because the
        // overlap check filters by employee on every write.
