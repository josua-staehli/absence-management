...
///     asked across the module boundary, and whether the values survive the trip through the
///     database.
/// </summary>
public class AbsenceRequestUseCaseTests
{
    ...
    ///     the employees module can answer.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     No foreign key spans the two databases, so this is a state the absences module has to
    ///     survive rather than one it can rule out.
    /// </summary>
    [Fact]
    public async Task The_list_stays_readable_when_the_employees_module_does_not_know_the_id()
    {
        await using var fixture = await AbsencesFixture.CreateAsync();
